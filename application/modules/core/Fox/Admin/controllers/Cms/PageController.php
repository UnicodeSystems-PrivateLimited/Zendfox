 ...
 *
 * Class Admin_Cms_PageController
 * 
 ...
 */
class Admin_Cms_PageController extends Fox_Admin_Controller_Action {
    ...
    function indexAction() {
        $this->loadLayout();
        $view = Fox::getView('cms/admin/page');
        if ($view) {
            $this->_addContent($view);
        }
        $view->setHeaderText('Manage Pages');
        $this->renderLayout();
    }
    ...
    public function editAction() {
        $this->loadLayout();
        $view = Fox::getView('cms/admin/page/add');
        if ($view) {
            $this->_addContent($view);
        }
        ...
        if ($id) {
            ...
            $model->load($id);
            if ($model->getId()) {
                $view->setHeaderText('Edit Page "' . $model->getTitle() . '"');
            } else {
                $view->setHeaderText('Add Page');
            }
        } else {
            $view->setHeaderText('Add Page');
        }
        ...
        if ($this->getRequest()->isPost()) {
            try {
                ...
                if (($id = $this->getRequest()->getParam('id'))) {
                    ...
                    $data['created'] = Fox_Core_Model_Date::getCurrentDate('Y-m-d H:i:s');
                }
    ...
    public function ajaxTableAction() {
        $view = Fox::getView('cms/admin/page/table');
        if ($view) {
