...
class Uni_Core_Gapi
{
  ...
  {
    $response = $this->httpRequest(Uni_Core_Gapi::account_data_url, array('start-index'=>$start_index,'max-results'=>$max_results), null, $this->generateAuthHeader());
  ...
  public function requestReportData($report_id, $dimensions, $metrics, $sort_metric=null, $filter=null, $start_date=null, $end_date=null, $start_index=1, $max_results=30)
  {
    ...
    $parameters['prettyprint'] = Uni_Core_Gapi::dev_mode ? 'true' : 'false';

    $response = $this->httpRequest(Uni_Core_Gapi::report_data_url, $parameters, null, $this->generateAuthHeader());
  ...
  protected function reportObjectMapper($xml_string)
  {
    ...
    foreach($xml->entry as $entry)
    {
      ...
      $results[] = new Uni_Core_GapiReportEntry($metrics,$dimensions);
    }
  ...
  protected function authenticateUser($email, $password)
  {
    $post_variables = array(
      ...
      'Email' => $email,
      'Passwd' => $password,
      'source' => Uni_Core_Gapi::interface_name,
      'service' => 'analytics'
    );

    $response = $this->httpRequest(Uni_Core_Gapi::client_login_url,null,$post_variables);
  ...
  {
    $interface = Uni_Core_Gapi::http_interface;

    if(Uni_Core_Gapi::http_interface =='auto')
    {
  ...
  public function __call($name,$parameters)
  {
    ...
    $parameter_key = Uni_Core_Gapi::array_key_exists_nc($name,$this->report_root_parameters);
    ...
    $aggregate_metric_key = Uni_Core_Gapi::array_key_exists_nc($name,$this->report_aggregate_metrics);
...
class gapiAccountEntry
{
  ...
  public function __call($name,$parameters)
  {
    ...
    $property_key = Uni_Core_Gapi::array_key_exists_nc($name,$this->properties);
